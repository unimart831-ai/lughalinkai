...
| Metric | Value |
|--------|------:|
| Unique PSA records (after dedupe) | **4,645** input → **559** strict-clean |
| Quarantined non-PSA / junk / long reports | **4,086** |
| Approx. English sentences in strict-clean corpus | **~4,400** |
| Domains covered | 5 / 5 (imbalanced) |
| Documented active sources in registry | **53** (≥10 required) |
| Rows with Kiswahili text filled | 3 in strict-clean |
| Target-language columns | Dholuo / Ekegusii / Somali placeholders |

**Strict PSA gate (pre-Product 2):** max **300** tokens, classifier score ≥ **0.40**, reject nav dumps / NGO stubs / country briefs / partnership PR / off-topic news. Re-run: `python scripts/strict_clean_psas.py`.

**Course threshold:** ≥5,000 parallel sentences — earlier broad merge met sentence volume; **strict-clean is quality-first (~4.4k sentences in 559 PSA records)**. Next step is collect more *real* PSAs (especially Education/Agriculture), not loosen the gate.
